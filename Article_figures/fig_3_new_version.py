...
def plot_population(path, s_in_movement = np.array([0.4, 1.2, 5.0])):
    lambda0 = 461e-9
    k = 2*np.pi/lambda0
    
    with open(os.path.join(path,f"atomic_movement_s={s_in_movement[0]:.1f}.npy"), 'rb') as f:
        density_m = np.load(f)
...
if __name__ == "__main__":
    ...
    # plot first the smallest times
    time_step = 0.1e-6 # Time step in seconds
    path = 'C:\\Users\\Raul\\Documents\\Repositories\\2026_mCBS_saturated\\mCBS_Atomic_Movement\\data\\smaller_time_step'
    time_points = [0,4,10]
    plot_columns(path, time_step, time_points)

    # now the longest ones

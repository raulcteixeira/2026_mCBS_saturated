...
def plot_columns(path, delimiter=','):
    ...
    linestyles = ['-', '--', '-.', ':', (0, (1, 1)),  (0, (3, 1.5, 1, 1.5, 1, 1.5)), (0, (3, 1, 1, 1))]
    ...
    plt.plot(x, data[:, 1], label=header[1], linestyle=linestyles[0], linewidth=2)
    ...
    header[1] = "Including motion"

    plt.plot(x, data[:, 1], label=header[1], linestyle=linestyles[3], color = 'red',linewidth=2)

    filename = os.path.join(path,"Contrast_movement_1us.txt")
    # Read header (column names)
    with open(filename, 'r') as f:
        header = f.readline().strip().split(delimiter)
    ...
    header[1] = f"Including motion, t = 1 $\mu$s"

    plt.plot(x, data[:, 1], label=header[1], linestyle=linestyles[5], color = 'olive', linewidth=2)
...
if __name__ == "__main__":
    ...
    plot_columns(path)
    plt.errorbar(s_exp, Contrast_exp, xerr=s_err, yerr=Contrast_err, fmt='o', color = (140/255, 86/255, 75/255), label='Experimental')

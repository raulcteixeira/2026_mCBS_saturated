...
dz = z(2)-z(1);
N_points_zsw = 20;                                   % Number of points for z to perform the integral just on a period of the standing wave
zsw = linspace(0,lambda/ cos(theta_0)*(N_points_zsw-1)/N_points_zsw,N_points_zsw);
[R,Z,Zsw] = meshgrid(r,z,zsw);
...
Zsw = Zsw + pi/(2*k*cos(theta_0)) - rem(Z,(pi/k/cos(theta_0)));
...
for ii = 1: N_points_z
    rho_center(ii,:,:) = rho(ii,:,:)./rho_sum_BL(1,1,1)/dz*b0;  %Normalized for Beer-Lambert lawZ
end

Intensities = zeros (N_points_Omega,N_point_theta);

fidss = fopen( 'comparison_Python_Matlab_codes/ss_Mathilde.txt', 'w' );
...
for j = 1:N_points_Omega    
    ...
    Z_reflected = flip(Z, 1);
    s_0_Z_reflected = zeros(size(Z));
    s_0_Z_reflected(1,:,:) = s_0_Z(end,:,:);

    rho_center_reflected = flip(rho_center,1);
...
%     plot(s_0_Z(:,50,1))
    ...
    Omega_Z_reflected = flip(sqrt(s_0_Z_reflected/2)*G_Sr,1);
    ...
    for i = 1:N_point_theta
        ...
        I(i) = sum(sum(R_sum.*rho_sum.*ss_avg));  % Intensity as a function of theta (fringes);
        if j == 1
            fprintf(fidss, '%.5f  ', theta(i));
            fprintf(fidss, '%.3f  ', ss_avg(N_points_z/2+ 20,N_points_r/2));
            fprintf(fidss,'\r\n');
        end
    end
%     figure(20)
...
disp(rho(N_points_z/2,1,1))
disp(rho(N_points_z/2,N_points_r,1))
...
disp(sum(rho_center(:,1,1))*dz)
disp(sum(rho_center(:,N_points_r,1))*dz)
...
fidOmega = fopen( 'comparison_Python_Matlab_codes/Omega_Mathilde.txt', 'w' );
fidss_elastic = fopen( 'comparison_Python_Matlab_codes/ss_Mathilde_elastic.txt', 'w' );
...
for j = 1:N_points_Omega    
    ...
    figure(100)
    plot(Omega_Z(:,1,N_points_zsw/2)); hold on;
    plot(Omega_Z_reflected(:,1,N_points_zsw/2)); hold on;
    
    fprintf(fidOmega, '%.3f  ', s_0(j)/4);
    fprintf(fid, '%.3f  ', Omega_total(N_points_z/2+20,1,:));
    fprintf(fid,'\r\n');
...
%         ss_avg = sum(s./(1+s).*(1 + real(exp(-sqrt(-1)*2*k*(Z+Zsw)*cos(theta(i))).*g1_total)),3);
        ss_avg = sum(s./(1+s).*(1 + cos(2.*k.*(Z+Zsw).*cos(theta(i)))),3);
        I(i) = sum(sum(R_sum.*rho_sum.*ss_avg));  % Intensity as a function of theta (fringes);
        if j == 1
            fprintf(fidss_elastic, '%.5f  ', theta(i));
            fprintf(fidss_elastic, '%.3f  ', ss_avg(N_points_z/2+ 20,N_points_r/2));
%             to_print = s./(1+s).*(1 + cos(2.*k.*(Z+Zsw).*cos(theta(i))));
%             fprintf(fidss_elastic, '%.3f  ', to_print(N_points_z/2,N_points_r/2,:));
            fprintf(fidss_elastic,'\r\n');
        end
    end
%     figure(20)
%     hold on
...
for n=1:N_points_Omega
    fprintf(fid, '%.3f  ', s_0(n)/4);
    fprintf(fid, '%.3f  ', b_0_final(n));
    fprintf(fid, '%.3f  ', b_0_final_reflected(n));
    fprintf(fid,'\r\n');
end
fclose(fid);
